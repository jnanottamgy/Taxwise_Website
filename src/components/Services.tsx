...
const CARD =
  "border border-paper-12 bg-ink-2/30 hover:border-paper-40 h-full flex flex-col";

function Points({ points, align = "bottom" }: { points: readonly string[]; align?: "top" | "bottom" }) {
  return (
    <ul className={`space-y-2.5 ${align === "bottom" ? "mt-auto pt-8" : "pt-1"}`}>
      {points.map((point) => (
        <li key={point} className="flex items-baseline gap-3 font-mono text-[0.6875rem] tracking-[0.04em] text-paper-64">
    ...
    >
      ...
      <div className="mt-20 grid gap-px bg-paper-12 lg:grid-cols-3">
        ...
        <Reveal className="lg:col-span-3" as="div">
          <LightCard as="article" className={`${CARD} border-0`}>
            <div className="grid gap-10 p-8 lg:grid-cols-[1fr_18rem] lg:p-12">
              <div>
                ...
                <p className="mt-5 max-w-[46ch] text-[0.9375rem] leading-[1.7] text-mist">
                  {feature.detail}
                </p>
              </div>
        ...
        {rest.map((service, i) => (
          <Reveal key={service.slug} delay={i * 0.06} as="div" className="h-full">
            <LightCard as="article" className={`${CARD} border-0 p-8 lg:p-10`}>
              <h3 className="font-display text-[1.75rem] leading-none text-paper">{service.name}</h3>
